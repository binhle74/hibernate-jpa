package com.binhle.hjpa.model.collections.basictypes.bagofstringsorderby;
...
public class BagOrderByTest extends BaseTest {
    ...
    public void testBagImagesOrderBy() {
        ...
        String expected = item.getImages().stream().sorted(comp.reversed()).reduce("", this::concat);
        this.beginTransaction().persist(item).commitTransaction().closeEntityManager();

        Item savedItem = this.find(Item.class, item.getId());
        String savedImages = savedItem.getImages().stream().reduce("", this::concat);
